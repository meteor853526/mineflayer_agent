    ...
    match data['type']:
        ...
        case messageType.chat:
            ...
            if data['message'] == 'action':
                ...
                data['message'] = Jeff_instance.observation(data)
        ...
        case messageType.system:
            ...
            if data['message'] == 'system:JobFinish':
                ...
                actionName = Jeff_instance.actionNameById(data['JobID'])
                Jeff_instance.buffer.put(f"({data['time']}) : You have done '{actionName}'")
            if data['message'] == 'system:help':
                data['message'] = Jeff_instance.conversationForHelp(data)
                data['whisperMsg'] = data['message']
                sio.emit('agi',data)
                
        case messageType.observe:

            if 'match_item' in data:
                quest = Jeff_instance.check_requestState(data['receiverName'],data['item_name'],data['item_id'])
                if quest['state'] == True :
                    data['message'] = Diedie_instance.action("Give you the "+data['item_name']+"for your request", data)
                    data['state'] = "waiting"
                    data['request_id'] = quest['request_id'] 
                    sio.emit('agi',data)
                    Jeff_instance.disable_requestState(data['receiverName'],data['item_name'])
            else:
                match data['agentState']:
                    ...
                    case state.schedule:
                        ...
                        Jeff_instance.clearJobQueue()
                        Jeff_instance.buffer.put(f"({data['time']}) : You noticed that '{data['observation']}'")
                        data['message'] = Jeff_instance.observation(data)
    ...
    match data['type']:
        ...
        case messageType.chat:
            ...
            if data['message'] == 'action':
                ...
                data['message'] = Diedie_instance.observation(data)
        ...
        case messageType.system:
            ...
            if data['message'] == 'system:JobFinish':
                ...
                actionName = Diedie_instance.actionNameById(data['JobID'])
                Diedie_instance.buffer.put(f"({data['time']}) : You have done '{actionName}'")
            if data['message'] == 'system:help':
                data['message'] = Diedie_instance.conversationForHelp(data)
                data['whisperMsg'] = data['message']
                sio.emit('agi',data)
            ...
            if 'match_item' in data:
                ...
                match data['agentState']:
                    ...
                    case state.schedule:
                        ...
                        # Diedie_instance.clearJobQueue()
                        Diedie_instance.buffer.put(f"({data['time']}) : You noticed that '{data['observation']}'")
                        data['message'] = Diedie_instance.observation(data)
