...
class findStone_sword extends BaseBehavior {
    constructor(bot, targets) {
        super(bot, 'findStone_sword', targets);
        this.working = true
    }
    async onStateEntered() {
        ...
        while(chests.length !== 0) {
          ...
          await sleepwait(2000)
          var target = chest_window.containerItems().filter(item => item.name.includes("stone_sword"))[0];
          await sleepwait(2000)
          if(target){
            await this.withdrawItem(chest_window,'stone_sword',1);
            await sleepwait(2000)
  ...
  function have_stone_sword(bot){
    if(bot.inventory.items().filter(item => item.name.includes("stone_sword"))[0])
      return true
    return false
  }
...
class Return_schedule extends BaseBehavior {
  ...
  function createFindStone_sword (bot, targets) {
    const enter = new BehaviorIdle();
    const exit = new BehaviorIdle();

    const findSword = new findStone_sword(bot, targets);
    const socket_schedule = new Socket_schedule(bot,targets,"find stone_sword"," stone_sword","5. go to the smelter and search ' stone_sword' from surrounding chest\n6. go to the smelter and craft 'stone_sword'");
    const return_schedule = new Return_schedule(bot, targets, "find stone_sword", "stone_sword", "1. killing");
    const transitions = [
      new StateTransition({
        parent: enter,
        child: findSword,
      ...
      new StateTransition({
        ...
        shouldTransition: () => findSword.isFinished() && !have_stone_sword(bot) && JobCheck(findSword.isFinished()) == true,
      }),
      new StateTransition({
  ...
  exports.createFindStone_sword = createFindStone_sword