...
class findFishing_rod extends BaseBehavior {
    constructor(bot, targets) {
        super(bot, 'findFishing_rod', targets);
        this.working = true
    }
    async onStateEntered() {
        ...
        while(chests.length !== 0) {
          ...
          await sleepwait(2000)
          var target = chest_window.containerItems().filter(item => item.name.includes("fishing_rod"))[0];
          await sleepwait(2000)
          if(target){
            await this.withdrawItem(chest_window,'fishing_rod',1);
            await sleepwait(2000)
  ...
  function have_fishing_rod(bot){
    if(bot.inventory.items().filter(item => item.name.includes("fishing_rod"))[0])
      return true
    return false
  }
...
class Return_schedule extends BaseBehavior {
  ...
  function createFindFishing_rod (bot, targets) {
    ...
    const transitions = [
      ...
      new StateTransition({
        ...
        shouldTransition: () => FindRod.isFinished() && !have_fishing_rod(bot) && JobCheck(FindRod.isFinished()) == true,
        onTransition: () => {
          bot.prev_jobs.push("find fishing_rod")
        }
      }),
      new StateTransition({
  ...
  exports.createFindFishing_rod = createFindFishing_rod