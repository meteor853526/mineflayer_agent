const {
  ...
  function createFeedChickenState(bot, targets) {
    ...
    const socket_schedule = new Socket_schedule(bot,targets,"wheat_seeds","I don't have the wheat_seeds")
    ...
    const transitions = [
        ...
        new StateTransition({
            ...
            onTransition: () =>{
              bot.chat("I've to go to find Guild.");
            }
        }),
        new StateTransition({
          parent: goGuild,
          child: socket_schedule,
            shouldTransition: () =>goGuild.isFinished() && !have_wheat_seeds(bot) && bot.agentState == 'schedule' && JobCheck(goGuild.isFinished()) == true,
            onTransition: () => {
        ...
        new StateTransition({
          parent: goGuild,
          child: socket_chat,
            shouldTransition: () =>goGuild.isFinished() && !have_wheat_seeds(bot) && bot.agentState == 'chat' && JobCheck(goGuild.isFinished()) == true,
            onTransition: () => {
              bot.chat("I didn't found wheat_seeds in chest");
            }
        }),
