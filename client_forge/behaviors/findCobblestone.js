...
class findCobblestone extends BaseBehavior {
  constructor(bot, targets) {
      super(bot, 'findCobblestone', targets);
      this.working = true
  }
  async onStateEntered() {
      ...
      while(chests.length !== 0) {
        ...
        await sleepwait(2000)
        if(target){
          var chest_stone = target.count
          await this.withdrawItem(chest_window,'cobblestone',chest_stone);
          await sleepwait(2000)
...
function have_cobble_stone(bot){
...
function createFindCobble_stone (bot, targets) {
  const enter = new BehaviorIdle();
  const exit = new BehaviorIdle();

  const findStone = new findCobblestone(bot, targets);
  const socket_schedule = new Socket_schedule(bot,targets,"find cobblestone"," cobblestone","5. go to the smelter and search cobblestone from surrounding chest");
  ...
  const transitions = [
    ...
    new StateTransition({
      ...
      shouldTransition: () => findStone.isFinished() && !have_cobble_stone(bot) && JobCheck(findStone.isFinished()) == true,
      onTransition: () => {
        bot.prev_jobs.push("find cobblestone")
      }
    }),
    new StateTransition({
...
exports.createFindCobble_stone = createFindCobble_stone;