...
class findOak_log extends BaseBehavior {
    constructor(bot, targets) {
        super(bot, 'findOak_log', targets);
        this.working = true
    }
...
class Return_schedule extends BaseBehavior {
  ...
  function createFindOak_log (bot, targets) {
    ...
    const transitions = [
      ...
      new StateTransition({
        ...
        shouldTransition: () => FindLog.isFinished() && !have_oak_log(bot) && JobCheck(FindLog.isFinished()) == true,
        onTransition: () => {
          bot.prev_jobs.push("find oak_log")
        }
      }),
      new StateTransition({
  ...
  exports.createFindOak_log = createFindOak_log