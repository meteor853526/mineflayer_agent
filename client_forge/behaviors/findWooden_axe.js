...
class findWooden_axe extends BaseBehavior {
    constructor(bot, targets) {
        super(bot, 'findWooden_axe', targets);
        this.working = true
    }
    async onStateEntered() {
        ...
        while(chests.length !== 0) {
          ...
          await sleepwait(2000)
          var target = chest_window.containerItems().filter(item => item.name.includes("wooden_axe"))[0];
          await sleepwait(2000)
          if(target){
            await this.withdrawItem(chest_window,'wooden_axe',1);
            await sleepwait(2000)
  ...
  exports.findWooden_axe = findWooden_axe;