const {
  ...
  class BehaviorBurnCharcoal extends BaseBehavior {
    ...
    async onStateEntered() {
        ...
        putInFurnace('fuel', 'spruce_planks', 1, this.bot)
        await sleep(2000)
        putInFurnace('input', 'spruce_log', 1, this.bot)
  ...
  class FindPlankfromChest extends BaseBehavior {
    ...
    async onStateEntered() {
        ...
        while(chests.length !== 0) {
          ...
          await sleepwait(2000)
          if(target){
            await this.withdrawItem(chest_window,'spruce_planks',1);
            await sleepwait(2000)
            await this.bot.closeWindow(chest_window)
            break;
          }
  ...
  function createBurnCharcoalState(bot, targets) {
    ...
    const socket_schedule_log = new Socket_schedule(bot,targets,"spruce_log","I don't have spruce_log")
    const socket_chat_log = new Socket_chat(bot,targets,"spruce_log","I don't have spruce_log, so I can't burn a charcoal")
    const socket_schedule_planks = new Socket_schedule(bot,targets,"spruce_planks","I don't have spruce_planks")
    const socket_chat_planks = new Socket_chat(bot,targets,"spruce_planks","I don't have spruce_planks, so I can't burn a charcoal")
    const transitions = [
        new StateTransition({
            parent: enter,
            child: findLog,
            shouldTransition: () => !have_log(bot),
            onTransition: () => {
        ...
        new StateTransition({
            parent: enter,
            child: findPlanks,
            shouldTransition: () => !have_planks(bot),
            onTransition: () => {
        ...
        new StateTransition({
            parent: enter,
            child: BurnCharcoal,
            shouldTransition: () => have_planks(bot) && have_log(bot),
        }),
        new StateTransition({
            parent: findLog,
            child: findPlanks,
