...
class findOak_planks extends BaseBehavior {
    constructor(bot, targets) {
        super(bot, 'findOak_planks', targets);
        this.working = true
    }
...
class Return_schedule extends BaseBehavior {
  ...
  function createFindOak_planks (bot, targets) {
    const enter = new BehaviorIdle();
    const exit = new BehaviorIdle();

    const FindPlanks = new findOak_planks(bot, targets);
    const socket_schedule = new Socket_schedule(bot,targets,"find oak_planks"," oak_planks","5. go to loggingCamp and searching oak_planks from surrounding chest\n6. woodtransform");
    const return_schedule = new Return_schedule(bot, targets, "find oak_planks", "oak_planks", "1. craft sticks\n2. burn charcoal");
    const transitions = [
      new StateTransition({
        parent: enter,
        child: FindPlanks,
      ...
      new StateTransition({
        ...
        shouldTransition: () => FindPlanks.isFinished() && !have_oak_planks(bot) && JobCheck(FindPlanks.isFinished()) == true,
        onTransition: () => {
          bot.prev_jobs.push("find oak_planks")
        }
      }),
      new StateTransition({
  ...
  exports.createFindOak_planks = createFindOak_planks