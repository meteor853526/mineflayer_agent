...
class BehaviorWoodTransform extends BaseBehavior {
    ...
    async onStateEntered() {
        ...
        // to crafting spruce_planks
        var name = 'spruce_planks'
        var amount = 1
...
class FindlogfromChest extends BaseBehavior {
    ...
    async onStateEntered() {
        ...
        while(chests.length !== 0) {
            ...
            await sleepwait(2000)
            var target = chest_window.containerItems().filter(item => item.name.includes("spruce_log"))[0];
            await sleepwait(2000)
            if(target){
                await this.withdrawItem(chest_window,'spruce_log',1);
                await sleepwait(2000)
...
class putplankBackToChest extends BaseBehavior {
    ...
    async onStateEntered() {
        ...
        var spruce_planks = mcData.itemsByName['spruce_planks'].id;
        await sleepwait(2000)
        console.log("?????????????????")
        if(await this.bot.inventory.findInventoryItem(spruce_planks)){
          console.log("????")
          var stick_number = await this.bot.inventory.findInventoryItem(spruce_planks).count
          ...
          await sleepwait(2000)
          await this.depositItem(chest_window,'spruce_planks',plank_chest_position);
          await sleepwait(2000)
          await this.bot.closeWindow(chest_window)
        }
...
function createWoodTransformState(bot, targets) {
    ...
    const transitions = [
        new StateTransition({
            parent: enter,
            child: transform,
            shouldTransition: () => have_spruce_log(bot),
        }),
        new StateTransition({
            parent: enter,
            child: find_log,
            shouldTransition: () => !have_spruce_log(bot),
            onTransition: () => {
              bot.chat("No spruce_log on my body");
          }
        }),
        new StateTransition({
            parent: transform,
        ...
        new StateTransition({
          parent: find_log,
          child: transform,
          shouldTransition: () => find_log.isFinished() && have_spruce_log(bot) && JobCheck(find_log.isFinished()) == true,
          onTransition: () => {
            bot.chat("I found spruce_log in chest");
          }
        }),
        new StateTransition({
            parent: find_log,
            child: socket_schedule,
            shouldTransition: () =>find_log.isFinished() && !have_spruce_log(bot) && bot.agentState == 'schedule' && JobCheck(find_log.isFinished()) == true,
            onTransition: () => {
              bot.chat("I didn't found spruce_log in chest");
            }
        }),
        ...
        new StateTransition({
            parent: find_log,
            child: socket_chat,
            shouldTransition: () => find_log.isFinished() && !have_spruce_log(bot) && bot.agentState == 'chat' && JobCheck(find_log.isFinished()) == true,
            onTransition: () => {
              bot.chat("I didn't found spruce_log in chest");
            }
        }),
