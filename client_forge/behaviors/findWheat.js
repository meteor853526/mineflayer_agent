...
class findWheat extends BaseBehavior {
    constructor(bot, targets) {
        super(bot, 'findWheat', targets);
        this.working = true
    }
    async onStateEntered() {
        ...
        while(chests.length !== 0) {
          ...
          await sleepwait(2000)
          if(target){
            await this.withdrawItem(chest_window,'wheat',1);
            await sleepwait(2000)
  ...
  function have_wheat(bot){
    if(bot.inventory.items().filter(item => item.name.includes("wheat"))[0])
      return true
    return false
  }
...
class Return_schedule extends BaseBehavior {
  ...
  function createFindWheat (bot, targets) {
    ...
    const transitions = [
      ...
      new StateTransition({
        ...
        shouldTransition: () => find_wheat.isFinished() && !have_wheat(bot) && JobCheck(find_wheat.isFinished()) == true,
        onTransition: () => {
          bot.prev_jobs.push("find wheat")
        }
      }),
      new StateTransition({
  ...
  exports.createFindWheat = createFindWheat