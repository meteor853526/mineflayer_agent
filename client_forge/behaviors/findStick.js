...
class findStick extends BaseBehavior {
  constructor(bot, targets) {
      super(bot, 'findStick', targets);
      this.working = true
  }
  async onStateEntered() {
    ...
    while(chests.length !== 0) {
      ...
      await sleepwait(2000)
      if(target){
        var chest_stick = target.count
        await this.withdrawItem(chest_window,'stick',chest_stick);
        await sleepwait(2000)
...
function createFindStick(bot, targets) {
  const enter = new BehaviorIdle();
  const exit = new BehaviorIdle();

  const FindStick = new findStick(bot, targets);
  const socket_schedule = new Socket_schedule(bot,targets,"find stick"," stick","5. go smeltingPlant and find stick");
  ...
  const transitions = [
    ...
    new StateTransition({
      ...
      shouldTransition: () => FindStick.isFinished() && !have_stick(bot) && JobCheck(FindStick.isFinished()) == true,
      onTransition: () => {
        bot.prev_jobs.push("find stick")
      }
    }),
    new StateTransition({
...
exports.createFindStick = createFindStick;